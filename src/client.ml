...
let read_header ic =
  Lwt_io.read_line ic
  >>= function "" -> return None | s -> return (Some (parse_header s))
...
let read_headers ic =
  let rec loop ic acc =
    read_header ic
    >>= function
    | None -> return (List.rev acc) | Some x -> loop ic (`Other x :: acc)
  in
  loop ic []
...
let request_inet ~server_name ~port req =
  ...
  let finally_ () = Lwt_unix.close sock in
    ...
    (fun () ->
      ...
      >>= fun response -> finally_ () >>= fun () -> return response )
...
let request_sock ~socket_filename req =
  ...
  let finally_ () = Lwt_unix.close sock in
    ...
    (fun () ->
      Lwt_unix.(connect sock @@ Unix.ADDR_UNIX socket_filename)
      >>= fun () ->
      send_request sock req
      >>= fun () ->
      receive_response sock
